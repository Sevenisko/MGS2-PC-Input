...
int mouseMovement_X = 0;
int mouseMovement_Y = 0;
...
float fl_table[12] = { 1000.0, -1400.0, 900.0, 2000.0, -1000.0, 900.0, 3700.0, 950.0, 500.0, 2250.0, 5500.0, 0.0 };

float pos_corrections_table[4] = { 0.0, 1.0, 0.0, 0.0 };
...
float Sensitivity = (float)0.6; // 1.3333334
bool Invert_X     = false;
bool Invert_Y     = false;
...
void math_stuff_inner(float* output, float value) {
    ...
    if (value >= -1.5707963 /*-1.5708*/) {
        if (value > 1.5707963 /*1.5708*/) {
            ...
            b = a * a;
            c = (float)-(float)((float)(a * a) * (float)((float)((float)(a * a) * (float)((float)((float)(a * a) * (float)((float)((float)(a * a) * 0.000024802) + -0.0013889)) + 0.0416667 /*0.041667*/)) + -0.5)) - 1.0;
        } else {
            b = value * value;
            a = value;
            c = (float)((float)(value * value) * (float)((float)((float)(value * value) * (float)((float)((float)(value * value) * (float)((float)((float)(value * value) * 0.000024802) + -0.0013889)) + 0.0416667 /*0.041667*/)) + -0.5)) + 1.0;
        }
    } else {
        a = -3.1415927 /*-3.1416*/ - value;
        b = a * a;
    ...
    output[1] = c;
    *output   = a * (float)((float)(b * (float)((float)(b * (float)((float)(b * (float)((float)(b * 0.0000027557) + -0.00019841)) + 0.0083333)) + -0.1666667 /*-0.16667*/)) + 1.0);
}
...
void get_camera_new_pos_and_trg(NEW_TPS_CAMERA* ws) {
    ...
    if (ws->pos_cor_preset2 != 0.0) {
        ...
        float pos_mul_value = (float)(ws->pos_cor_preset2 / sqrtf((float)((float)(temp.vec[1] * temp.vec[1]) + (float)(temp.vec[0] * temp.vec[0])) + (float)(temp.vec[2] * temp.vec[2]))) + 1.0;
...
void set_camera_xy(NEW_TPS_CAMERA* ws) {
    ...
    float direction_delta = (float)((float)((float)(rx * 0.0078125) * 341.33331 /*341.33*/) * ws->xyz_mul_preset) + (float)(ws->direction_delta * 0.625);
...
void set_next_player_camera_special(CAMERA_OBJECT* cw, int flag) {
    ...
    case 4:
        ...
        break;
    case 1: calc_and_set_rot(cw->camera.position.vec, cw->camera.target.vec, cw->camera.rotate.vec, &cw->camera.track); break;
    case 2: calc_and_set_pos(cw->camera.position.vec, cw->camera.target.vec, cw->camera.rotate.vec, &cw->camera.track); break;
    }
}
...
void dump_camera_data(CAMERA_MGS2_PC* camera) {
    char buf[0x200];
    sprintf_s(buf, "Cam ID 0x%08X - Flag: 0x%X, Unk0: 0x%X, Priority: 0x%X, Enabled: 0x%X, Next: 0x%X, Ifi: 0x%X, Ifo: 0x%X, Iti: 0x%X, Ito: 0x%X, Channel ID: 0x%X, Interp ID: 0x%X\n", camera->camera_id, camera->flag, camera->unk0, camera->priority, camera->enabled, camera->next,
              (DWORD)camera->interpolate_func_in, (DWORD)camera->interpolate_func_out, camera->interpolate_time_in, camera->interpolate_time_out, camera->channel_id, camera->interpolate_id);
    OutputDebugStringA(buf);
                        ...
                        for (int i = 0; i < 9; ++i) {
                            ...
                            while (1) {
                                ...
                                if (camera->enabled && camera != &cw->camera && !is_patchable_camera(camera)) {
                                    if (camera->camera_id == 0x0072EF36 /* FPS mode */ || camera->camera_id == 0x001BF6B8 /* FPS mode with weapon */ || camera->camera_id == 0x0051BBC9 /* ITEM_SCOPE_2 */
                                    ) {
                                        fps_mode = true;
                                    }
...
void __cdecl convert_stick_to_button(void* arg0, void* arg1, void* arg2, int arg3) {
    ...
    if (is_fps_mode || (active_weapon_id == WEAPON_COOLANT) || (active_weapon_id == WEAPON_D_MIC) || (active_weapon_id == WEAPON_D_MIC_2) || (active_weapon_id == WEAPON_PSG1) || (active_weapon_id == WEAPON_PSG1_T) || (active_weapon_id == WEAPON_STINGER)
        || (active_item_id == ITEM_SCOPE_2) || (*(UINT64*)0xF609C0 & 0x200) // pl_subject_demo
    ) {
...
int __cdecl check_if_snake_is_next_to_wall_controls(BYTE* buttons) {
    if (IsEnabled && !IsPaddemo) {
        DAEMON_OBJECT* dw = (DAEMON_OBJECT*)*(DWORD*)Cameras_Daemon;
        if (dw) {
...
bool interpolate_and_rotate_stuff(CAMERA_CHANNEL_MGS2_PC* camera_channel) {
    ...
    if ((camera_channel->flag & 0x100) != 0) {
        if ((DWORD)camera_channel->interpolate_func == 0x0088AA90) {
            ...
            ((void(__cdecl*)(CAMERA_CHANNEL_MGS2_PC*, CAMERA_MGS2_PC*, CAMERA_MGS2_PC*))camera_channel->interpolate_func)(camera_channel, cam1, cam2);
        }
    } else if (cam2) {
...
void __cdecl camera_transitions_handler(DAEMON_OBJECT* dw) {
    if (IsEnabled && !IsPaddemo) {
        for (int chan_id = 0; chan_id < 2; chan_id++) {
            CAMERA_CHANNEL_MGS2_PC* camera_channel = &dw->channels[chan_id];
                ...
                if (*(DWORD*)(0xDB7044 + chan_id * 0x4A0)) {
                    ...
                    if (cam_next) {
                        ...
                        if (!*(DWORD*)0x118ADB0) {
                            ...
                            if (!(camera_channel->current.interpolate_id <= 2) && f > 0.1 && (*(DWORD*)(Cameras_Daemon + 0x92C + 0x910 * cur_channel_id) & 0x4000) == 0) {
                            ...
                            calc_and_set_rot(camera_channel->master.position.vec, camera_channel->master.target.vec, camera_channel->master.rotate.vec, &camera_channel->master.track);
...
int __cdecl check_if_snake_needs_to_jump(BYTE* a1) {
    if (IsEnabled && !IsPaddemo) {
        SVECTOR v12;
        float v13[2];
        float v14[4];
        ...
        if (v1 == 0x42D644 || v1 == 0xA5D059 || v1 == 0x8CD046 || (a1[0xDA] & 1) == 0) {
            return 0;
        }
...
int __cdecl start_paddemo(int char_id) {
...
void __cdecl paddemo_handler(BYTE* a1) {
...
int __cdecl diff_direction_for_swimming(int a1, int a2) {
    // Swimming fix
    DAEMON_OBJECT* dw = (DAEMON_OBJECT*)*(DWORD*)Cameras_Daemon;
...
int __cdecl pick_direction_for_swimming(int a1, int a2, int a3) {
    // Swimming fix
    DAEMON_OBJECT* dw = (DAEMON_OBJECT*)*(DWORD*)Cameras_Daemon;
    if (IsEnabled && dw && !IsPaddemo) {
        a2 += get_camera_direction(dw);
...
int __cdecl chara_func_0xDFADB() {
...
int __cdecl get_camera_direction_fix_for_hanging() {
    if (IsEnabled && !IsPaddemo) {
        return 0;
    } else {
...
int __cdecl diff_direction_for_hanging1(int a1, int a2) {
    // Hanging fix
    if (IsEnabled && !IsPaddemo) {
        a1 = 0x400;
    }
...
int __cdecl diff_direction_for_hanging2(int a1, int a2) {
    // Hanging fix
    if (IsEnabled && !IsPaddemo) {
        a1 = 0xC00;
    }
...
void __cdecl set_direction_for_PL_SubjectMove_wrap_with_set_stick_direction(void* a1) {
...
int __cdecl is_rightstick_used(void* a1) {
    if (IsEnabled) {
...
WORD __cdecl get_blade_direction(void* a1) {
    if (IsEnabled) {
...
int __cdecl check_if_special_blade_attack() {
    if (IsEnabled) {
...
int __cdecl diff_direction_for_blade_protection(int a1, int a2) {
...
int __cdecl raiden_event_msg_motion(void* a1, void* a2) {
    ...
    if (stage_name == 0x62353277 /* w25b */) {
        ...
        if (*(DWORD*)(*(DWORD*)((BYTE*)a2 + 8) + 20) == 0x403 && *(DWORD*)(*(DWORD*)((BYTE*)a2 + 8) + 8) == 0xC00) // It breaks hanging without this additional check
        {
            flag = true;
        }
    }
